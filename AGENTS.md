...
- Sessions need a SINGLE initiator: crossed simultaneous opens (both ends sending first, e.g. kernel chatter vs app payload) stall with half-open state on both sides — caught by `tun_ping` (forwarding only peer-addressed packets works around it). Go's `adopt_buffered` path is ported but evidently has a gap; needs a dedicated crossed-init loopback test + fix.
