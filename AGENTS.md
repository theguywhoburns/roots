...
- One `Router` serves any number of links through a `LinkSet` map (`serve_links`; `serve` is the one-entry case). Dead links are evicted, not fatal — survivors keep serving; the set empties only when the last link dies (preserves the old single-link `serve` contract). `prio`/`order` tiebreaks are recorded per link for the multi-peer future.
