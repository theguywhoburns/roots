...
- `src/address.rs` — key→IPv6 derivation. `src/handshake.rs` — link `meta` codec. `src/link.rs` — `Transport` trait + TCP dial/listen/handshake, backoff + `?maxbackoff=`/`?sni=` URI opts. `src/tls.rs` — `Tls` transport (rustls/ring, NoVerify like Go InsecureSkipVerify, rcgen self-signed listener). `src/ws.rs` — `Ws`/`Wss` transports (`ygg-ws` subprotocol, one binary message per flush, byte-stream reads; WSS layers WS on the TLS connector). `src/frame.rs` — ironwood link framing + uvarint/path helpers.
