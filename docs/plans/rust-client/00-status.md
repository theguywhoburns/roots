...
- [ ] Slice 8+ — `ws`/`quic`, TUN, admin, multi-peer conn map, debug `getSelf`/`getPeers`/`getTree` responders (typeSessionProto inbound currently dropped)

## Remaining work (as of 2026-09-06)
- Code cleanup: remove `ROOTS_DBG_DUMP`/`dump()` if no longer wanted (kept intentionally for now); MeshPhy boilerplate is copy-pasted across `http_fetch`/`mesh_tcp`/`irc_watch`/`tcp_loopback` test — factor into a shared example helper if a 4th consumer appears; `prio` field on PeerState is still `#[allow(dead_code)]` pending the multi-peer conn map.
- Slice 8+: `ws`/`quic` transports, TUN, admin socket, multi-peer conn map, debug `getSelf`/`getPeers`/`getTree` responders (typeSessionProto inbound currently dropped), Go-style lazy keepalive (we reply eagerly — harmless chatter).
