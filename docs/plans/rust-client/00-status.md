...
- **Library:** multi-peer conn map (DONE 10a/10b 2026-09-06), god-object split (DONE 2026-09-06: tree protocol `SigReq`/`SigRes`/`Announce` + parent selection out of `router.rs` into `src/tree.rs`; `router.rs` 1578→942 lines, orchestration only; 55 tests green), `wss` (DONE 2026-09-06, see Slice 10)/`quic` transports, Go-style lazy keepalive (we reply eagerly — harmless chatter), `getPaths`/`getSessions` query snapshots.
